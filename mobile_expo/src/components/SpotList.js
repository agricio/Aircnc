...
function SpotList({ tech, navigation }) {
    ...
    function handleNavigate(item) {
        const { _id, company, thumbnail_url, price } = item;
        navigation.navigate('Book', { _id, company, thumbnail_url, price });
    }

    return <View style={styles.container}>
        <Text style={styles.title}>Companies then use <Text style={styles.bold}>{tech}</Text></Text>
            ...
            renderItem={({ item }) => (
                <View style={styles.listItem}>
                    ...
                    <TouchableOpacity onPress={() => handleNavigate(item)} style={styles.button} >
                        <Text style={styles.buttonText}>Make Book</Text>
